...
public class WeiboLoginPlugin extends LoginPlugin {
    ...
    public User doRequest(HttpServletRequest request) {
        ...
        AssertUtils.notNull(accessToken, "missing accessToken");
        WeiBoUser weiboWeiBoUser = new Weibo().showUser(accessToken, thirdpartyId);
        Thirdparty thirdparty = new Thirdparty();
        thirdparty.setId(weiboWeiBoUser.getId());
        thirdparty.setLogo(weiboWeiBoUser.getAvatar_large());
        thirdparty.setNickName(weiboWeiBoUser.getScreen_name());
    ...
    public void callback(String action ,HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if ("login".contains(state)) {
            ...
            WeiBoAccessToken accessToken = weibo.getAccessToken(config.get("clientId"), config.get("secret"), code, config.get("redirectUri"));
