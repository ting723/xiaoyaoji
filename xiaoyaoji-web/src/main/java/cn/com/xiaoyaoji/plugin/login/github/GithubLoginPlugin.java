...
public class GithubLoginPlugin extends LoginPlugin {
    ...
    public User doRequest(HttpServletRequest request) {
        ...
        GitHubUser gitHubUser = github.getUser(accessToken);
        Thirdparty thirdparty = new Thirdparty();
        thirdparty.setId(gitHubUser.getId());
        thirdparty.setLogo(gitHubUser.getAvatar_url());
        thirdparty.setNickName(gitHubUser.getName());
        thirdparty.setType(getPluginInfo().getId());
        thirdparty.setEmail(gitHubUser.getEmail());
    ...
    public void callback(String action,HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if ("login".contains(state)) {
            ...
            String redirectUri = getPluginInfo().getConfig().get("redirectUri");
            BaseAccessToken accessToken = github.getAccessToken(clientId, secret, code, redirectUri);
            GitHubUser gitHubUser = github.getUser(accessToken.getAccess_token());

            request.setAttribute("thirdpartyId", gitHubUser.getId());
