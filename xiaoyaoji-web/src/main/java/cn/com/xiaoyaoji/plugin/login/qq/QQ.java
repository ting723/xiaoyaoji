...
public class QQ {
    ...
    public QQAccessToken getAccessToken(String code, String redirectURI){
        ...
        if(rs.contains("access_token")){
            ...
            if(matcher.find()){
                return new QQAccessToken(matcher.group(1),matcher.group(3),Long.parseLong(matcher.group(2)));
            }
            throw  new QQException(rs);
        }
        throw new QQException(rs);
